...
The first observation to be made is that our inter-annotator agreement did improve over time, as our specification was revised and the annotators became more familiar with the process, but it was never very good (removing MISSPELLING tags takes the overall agreement down to 15\%). However, between the two annotators, with one noticing one group of things and the other noting another, the gold standard turned out well. In the future, we should look at decreasing cognitive load for the annotators.

Part of how we can reduce cognitive load is by reducing the number of tags or simplifying in some way. Some of them, extracted as features, did not add any useful information; others, like the binary presence or absence of link tags, actually hurt. 
